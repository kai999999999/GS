Public Class frmReportSend
    ...
    Private Sub Button7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button7.Click
        ...
        Try
            With fpSize_Sheet1
                .RowCount = 0
                Sql = "SELECT * FROM [vProcessSend] WHERE Lot<>''"
                ...
                If TextBox4.Text.Trim <> "" Then
                    Sql = Sql & " AND [ชื่อส่งร้อย] = '" & TextBox4.Text.Trim & "'"
                End If
                    ...
                    If qResult(0).ToString.Trim.ToUpper = "SUCCESS" Then
                        If dtResult.Tables("Jobs").Rows.Count > 0 Then
                            For i As Integer = 0 To dtResult.Tables("Jobs").Rows.Count - 1
                                ...
                                .Cells(.RowCount - 1, 6).Text = dtResult.Tables("Jobs").Rows(i)("ชื่อคนตัด").ToString.Trim
                                .Cells(.RowCount - 1, 7).Text = dtResult.Tables("Jobs").Rows(i)("Cut")
                                .Cells(.RowCount - 1, 8).Text = dtResult.Tables("Jobs").Rows(i)("ส่งกล้า")
                                .Cells(.RowCount - 1, 9).Text = dtResult.Tables("Jobs").Rows(i)("น้ำหนักห่วง")
                                .Cells(.RowCount - 1, 10).Text = dtResult.Tables("Jobs").Rows(i)("น้ำหนักตัด")
                                .Cells(.RowCount - 1, 11).Text = dtResult.Tables("Jobs").Rows(i)("ชื่อส่งร้อย").ToString.Trim
                                .Cells(.RowCount - 1, 12).Text = dtResult.Tables("Jobs").Rows(i)("Send")
                                .Cells(.RowCount - 1, 13).Text = dtResult.Tables("Jobs").Rows(i)("น้ำหนักส่ง")
                                .Cells(.RowCount - 1, 14).Text = Format(CDate(dtResult.Tables("Jobs").Rows(i)("ReceiveDate")), "dd/MM/yyyy HH:mm:ss")
                                .Cells(.RowCount - 1, 15).Text = dtResult.Tables("Jobs").Rows(i)("น้ำหนักเก็บ")
                                .Cells(.RowCount - 1, 16).Text = dtResult.Tables("Jobs").Rows(i)("เศษห่วง")
                                .Cells(.RowCount - 1, 17).Text = dtResult.Tables("Jobs").Rows(i)("สรุปหาย")
                                .Cells(.RowCount - 1, 18).Text = dtResult.Tables("Jobs").Rows(i)("ฟุต")
                                .Cells(.RowCount - 1, 19).Text = dtResult.Tables("Jobs").Rows(i)("ห่วง")
                            Next
                        End If
                    End If
                End If
