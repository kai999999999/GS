Public Class frmProcessSaw
    ...
    Private Sub Button5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button5.Click
        If txtCode.Text.Trim <> "" Then
            clearText()
            'Sql = GetDescr("select SUM(หลังเชื่อม) from ProcessWeld Where [Lot]='" & txtLot.Text.Trim & "' And [LastStatus]=2 And Weld='" & txtCode.Text.Trim & "' And Barcode='" & txtBarcodeID.Text.Trim & "' And BarcodeText='" & txtBarcode.Text.Trim & "'")(2)
            'If IsNumeric(Sql) Then
                ...
                If qResult(0).ToString.Trim.ToUpper = "SUCCESS" Then
                    If dtResult.Tables("ProcessWeld").Rows.Count > 0 Then
                        For i As Integer = 0 To dtResult.Tables("ProcessWeld").Rows.Count - 1
                            ...
                            .Cells(.RowCount - 1, 4).Value = dtResult.Tables("ProcessWeld").Rows(i)("ก่อนเชื่อม")
                            .Cells(.RowCount - 1, 5).Value = dtResult.Tables("ProcessWeld").Rows(i)("น้ำประสาน")
                            .Cells(.RowCount - 1, 6).Value = dtResult.Tables("ProcessWeld").Rows(i)("หลังเชื่อม")
                            .Cells(.RowCount - 1, 7).Value = dtResult.Tables("ProcessWeld").Rows(i)("น้ำประสานเกิน")
                            .Cells(.RowCount - 1, 8).Value = dtResult.Tables("ProcessWeld").Rows(i)("เศษ")
                            .Cells(.RowCount - 1, 9).Value = dtResult.Tables("ProcessWeld").Rows(i)("หาย")
                        Next
                    End If
                End If
            End If
                ...
                If qResult(0).ToString.Trim.ToUpper = "SUCCESS" Then
                    If dtResult.Tables("ProcessSaw").Rows.Count > 0 Then
                        For i As Integer = 0 To dtResult.Tables("ProcessSaw").Rows.Count - 1
                            ...
                            .Cells(.RowCount - 1, 4).Value = dtResult.Tables("ProcessSaw").Rows(i)("น้ำหนักเลื่อย")
                            .Cells(.RowCount - 1, 5).Value = dtResult.Tables("ProcessSaw").Rows(i)("เลื่อยเสร็จ")
                            .Cells(.RowCount - 1, 6).Value = dtResult.Tables("ProcessSaw").Rows(i)("เศษ")
                            .Cells(.RowCount - 1, 7).Value = dtResult.Tables("ProcessSaw").Rows(i)("ผง")
                            .Cells(.RowCount - 1, 8).Value = dtResult.Tables("ProcessSaw").Rows(i)("หาย")
                            fpTotal_Sheet1.Cells(0, 0).Value = fpTotal_Sheet1.Cells(0, 0).Value + dtResult.Tables("ProcessSaw").Rows(i)("น้ำหนักเลื่อย")
                            fpTotal_Sheet1.Cells(0, 1).Value = fpTotal_Sheet1.Cells(0, 1).Value + dtResult.Tables("ProcessSaw").Rows(i)("เลื่อยเสร็จ")
                            fpTotal_Sheet1.Cells(0, 2).Value = fpTotal_Sheet1.Cells(0, 2).Value + dtResult.Tables("ProcessSaw").Rows(i)("เศษ")
                            fpTotal_Sheet1.Cells(0, 3).Value = fpTotal_Sheet1.Cells(0, 3).Value + dtResult.Tables("ProcessSaw").Rows(i)("ผง")
                            fpTotal_Sheet1.Cells(0, 4).Value = fpTotal_Sheet1.Cells(0, 4).Value + dtResult.Tables("ProcessSaw").Rows(i)("หาย")
                        Next
                    End If
                End If
            End If
    ...
    Private Sub Button3_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button3.Click
        If MessageBox.Show("You want to save data?", "Save", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = Windows.Forms.DialogResult.Yes Then
            ...
            Else
                ...
                Sql = Sql & "SET "
                Sql = Sql & "[น้ำหนักเลื่อย] = " & CDbl(TextBox10.Text.Trim)
                Sql = Sql & ",[เลื่อยเสร็จ] = " & CDbl(TextBox9.Text.Trim)
                Sql = Sql & ",[เศษ] = " & CDbl(TextBox20.Text.Trim)
                Sql = Sql & ",[ผง] = " & CDbl(TextBox6.Text.Trim)
    ...
    Private Sub Button7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button7.Click
        strCaption = "Finder - ข้อมูลเลื่อย"
        RetCount = 5
        RetField(0) = 0
        RetField(1) = 1
        RetField(2) = 2
